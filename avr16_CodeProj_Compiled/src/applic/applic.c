...
typedef enum {state_IDLE, state_WaitTO2Lock, state_ARM, state_WaitTO2UnLock } type_Lock_state;				
...
void f_LockFSM(void)
{
    ...
    {    
        ...
        case state_WaitTO2Lock:
            if (v_SwTimer_mS[c_TO_TIMER] == 0)
            { // finished TO, servo locked
        ...
        case state_WaitTO2UnLock:
            if (v_SwTimer_mS[c_TO_TIMER] == 0)
            { // finished TO, servo should have finished its motion
